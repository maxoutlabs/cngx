"""FastAPI application for cngx web UI."""
...
logger = logging.getLogger("cngx.server")

# ---------------------------------------------------------------------------
# Configuration from environment
# ---------------------------------------------------------------------------

MAX_REQUEST_BODY_BYTES = int(os.getenv("CNGX_MAX_REQUEST_BODY", str(2 * 1024 * 1024)))  # 2 MB
RATE_LIMIT_RPM = int(os.getenv("CNGX_RATE_LIMIT_RPM", "120"))  # 120 req/min default
...
async def lifespan(app: FastAPI):
    ...
    logger.info("cngx server starting (v%s)", __version__)
    db = get_database()
    logger.info("Database ready (%s traces)", db.get_stats().get("traces", 0))
    yield
    logger.info("cngx server shutting down, closing database")
    db.close()
...
_cors_origins = os.getenv(
    "CNGX_CORS_ORIGINS", "http://localhost:3000,http://localhost:8642"
).split(",")
app.add_middleware(
    CORSMiddleware,
...
async def capture_trace(request: CaptureRequest):
    ...
    tracer = CngxTracer(adapter=request.adapter, model=request.model)
