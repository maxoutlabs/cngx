...
from cngx.proxy.analysis import schedule_analysis
...
def _upstream_for_path(path: str) -> tuple[str, str, dict[str, str]] | None:
    ...
    if path.endswith("/v1/chat/completions") or path == "/v1/chat/completions":
        key = os.getenv("OPENAI_API_KEY") or os.getenv("CNGX_OPENAI_API_KEY")
        if not key:
            return None
...
def _task_id_from_request(request: Request, body: dict) -> str:
    header = request.headers.get("x-cngx-task-id") or request.headers.get("X-cngx-Task-Id")
    if header:
        return header
...
async def health(_: Request) -> JSONResponse:
    return JSONResponse({"status": "ok", "service": "cngx-proxy"})
