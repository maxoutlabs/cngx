# Regenerating terminal demo recordings
...
## Quickstart GIF (primary README demo)
...
`cogscope watch` was **not** recorded. The live TUI reads events from an in-process bus fed only by the proxy, and the proxy forwards to real provider APIs (no mock upstream). A honest recording would need live API keys and timing-dependent traffic. See AGENT_LOG in the private launch repo.
